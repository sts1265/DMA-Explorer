...
# Added 1er for French
ARTICLE_ONE_PATTERNS = [r"^Article\s+1\b", r"^Artikel\s+1\b", r"^Article\s+premier\b", r"^Article\s+1er\b"]
...
def parse_dma():
    ...
    for file_path in html_files:
        ...
        for el in elements:
            if el.name == 'p' and el.find_parent(['tr', 'div', 'p']): continue
            ...
            if current_art_num:
                ...
                if parsing_annex: label = "Annex"

                data.append({
                    'ID': current_sub_id, 'Type': row_type, 'Label': label,
