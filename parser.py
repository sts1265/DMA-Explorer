...
for file_path in html_files:
    ...
    current_art_num = ""
    parsing_annex = False
    passed_toc = False # To prevent double text

    # We iterate through paragraphs and table rows
    # Note: Only using 'p' and 'tr' to avoid duplication from 'div'
    for el in soup.find_all(['p', 'tr']):
        text = el.get_text(" ", strip=True).replace('\xa0', ' ').strip()
        if not text or len(text) < 2: continue

        # 1. SKIP TABLE OF CONTENTS
        # The actual content starts after the preamble. 
        # "HAS ADOPTED THIS REGULATION" is the marker.
        if "HAS ADOPTED THIS REGULATION" in text.upper() or "HAT FOLGENDE VERORDNUNG ERLASSEN" in text.upper():
            passed_toc = True
            continue

        # 2. DETECT RECITALS (Only before Articles start)
        if not passed_toc:
        ...
        if current_art_num:
            ...
            # Logic to match your Sheet 4 IDs
    ...
    if data:
        ...
        print(f"✅ Success: {len(df)} items for {lang_code}")
